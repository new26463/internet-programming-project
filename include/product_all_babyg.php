<?php
	...
	$select = "select * from product where prod_brand = 'BabyG' limit {$start} , {$perpage}";
...
<!-- /// ป๊อปอัพเวลากด details -->
  <div class="modal fade" id="myModal<?php echo $id;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
								...
								<center>
									<?php
										...
										while($data1 = mysqli_fetch_array($run1)){
											...
											if($color1 == 'white')$color1 = '#f5f5f5';
											else if($color1 == 'blue')$color1 = '#549bdd';
											else if($color1 == 'golden')$color1 = '#ddbf44';
											else if($color1 == 'gray')$color1 = '#a3a3a3';
											else if($color1 == 'green')$color1 = '#baffe1';
											else if($color1 == 'whiteblue')$color1 = '#a2caef';
											else if($color1 == 'darkblue')$color1 = '#0d375e';
											else if($color1 == 'orange')$color1 = '#f7764f';
									?>
									<span data-toggle="<?php echo $color2;?>" title="<?php echo $color2;?>">
										<img class="circle"  data-dismiss="modal" data-toggle="modal"  data-target="#myModal<?php echo $id1;?>" style="width:40px; height:40px; background:<?php echo $color1;?>;margin-left:5px;">
									</span>
									<?php } ?>
								</center>
								<hr width="90%" color="#ccc" style="margin:10px 0;">
	...
	<script type="text/javascript">
		...
		$('[data-toggle="gold"]').tooltip();
		$('[data-toggle="pink"]').tooltip();
		$('[data-toggle="yellow"]').tooltip();
		$('[data-toggle="blue"]').tooltip();
		$('[data-toggle="darkblue"]').tooltip();
		$('[data-toggle="white"]').tooltip();
		$('[data-toggle="whiteblue"]').tooltip();
		$('[data-toggle="purple"]').tooltip();
		$('[data-toggle="green"]').tooltip();
		$('[data-toggle="orange"]').tooltip();
	</script>
	<script>
...
<?php } ?>
<?php
	$select = "select * from product where prod_brand = 'BabyG'";
	$run = mysqli_query($con,$select);
...
<center style="clear:both">
	<ul class="pagination">
		<li>
			<a href="babyg.php?page=1" aria-label="Previous">
				<span aria-hidden="true">&laquo;</span>
			</a>
		</li>
		<?php for($i=1;$i<=$total_page;$i++){ ?>
			<li><a href="babyg.php?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
		<?php } ?>
		<li>
			<a href="babyg.php?page=<?php echo $total_page;?>" aria-label="Next">
